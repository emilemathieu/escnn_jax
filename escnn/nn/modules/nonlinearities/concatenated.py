...
from escnn_jax.group.representation import build_from_discrete_group_representation
...
class ConcatenatedNonLinearity(EquivariantModule):
    ...
    def forward(self, input: GeometricTensor) -> GeometricTensor:
        ...
        spatial_shape = input.tensor.shape[2:]
        
        # build the output tensor
        output = torch.empty(b, 2 * c, *spatial_shape, dtype=torch.float, device=input.tensor.device)
    ...
    def _transform_representation(representation: Representation) -> Representation:
        ...
        Transform an input :class:`~escnn.group.Representation` according to the concatenated non-linearity.
