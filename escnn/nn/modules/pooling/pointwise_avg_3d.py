...
__all__ = ["PointwiseAvgPool3D", "PointwiseAvgPoolAntialiased3D"]


class PointwiseAvgPool3D(EquivariantModule):
    ...
    def evaluate_output_shape(self, input_shape: Tuple[int, int, int, int, int]) -> Tuple[int, int, int, int, int]:
        ...
        do = (di + 2 * self.padding[2] - (self.kernel_size[2] - 1) - 1) / self.stride[2] + 1
...
class PointwiseAvgPoolAntialiased3D(EquivariantModule):
    ...
    def __init__(self,
        ...
        self.kernel_size = (filter_size, filter_size, filter_size)
        ...
        filter = filter.view(1, 1, filter_size, filter_size, filter_size).repeat((in_type.size, 1, 1, 1, 1))
