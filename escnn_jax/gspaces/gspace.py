...
class GSpace(ABC):
    ...
    def __init__(self, fibergroup: escnn_jax.group.Group, dimensionality: int, name: str):
        r"""
    ...
    def type(self, *representations: escnn_jax.group.Representation) -> escnn_jax.nn.FieldType:
        r"""
            Shortcut to build a :class:`~escnn.nn.FieldType`.
            This is equivalent to ``FieldType(gspace, representations)``.
        """
        return escnn_jax.nn.FieldType(self, representations)
    ...
    def featurefield_action(self, input: np.ndarray, repr: escnn_jax.group.Representation, element: escnn_jax.group.GroupElement, order: int =2) -> np.ndarray:
        r"""
    ...
    @property
    @abstractmethod
    def basespace_action(self) -> escnn_jax.group.Representation:
        r"""
    ...
    def _interpolate_transform_basespace(
            self,
            input: np.ndarray,
            element: escnn_jax.group.GroupElement,
            order: int = 2,
    ) -> np.ndarray:
        r"""
    ...
    @property
    def trivial_repr(self) -> escnn_jax.group.Representation:
        r"""
    ...
    @property
    def regular_repr(self) -> escnn_jax.group.Representation:
        r"""
    ...
    def quotient_repr(self, subgroup_id) -> escnn_jax.group.Representation:
        r"""
    ...
    def induced_repr(self, subgroup_id, repr: escnn_jax.group.Representation) -> escnn_jax.group.Representation:
        r"""
    ...
    def build_fiber_intertwiner_basis(self,
                                      in_repr: escnn_jax.group.Representation,
                                      out_repr: escnn_jax.group.Representation,
                                      ) -> escnn_jax.kernels.KernelBasis:
        r"""
        ...
        assert isinstance(in_repr, escnn_jax.group.Representation)
        assert isinstance(out_repr, escnn_jax.group.Representation)
        ...
        if (in_repr.name, out_repr.name) not in self._fields_intertwiners_basis_memory_fiber_basis:
            ...
            basis = escnn_jax.kernels.kernels_on_point(in_repr, out_repr)
    ...
    def build_kernel_basis(self,
                           ...
                           rings: List[float],
                           **kwargs) -> escnn_jax.kernels.KernelBasis:
        r"""
