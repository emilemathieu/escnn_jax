...
class TensorProductModule(EquivariantModule):
    ...
    inference: bool
    _C: int = eqx.field(static=True)
    hidden_type: FieldType = eqx.field(static=True)
    gspace: GSpace = eqx.field(static=True)
   
    def __init__(self, in_type: FieldType, out_type: FieldType, initialize: bool = True, inference: bool = False, *, key: PRNGKeyArray):
        r"""
        ...
        self.register_buffer("matrix", jnp.zeros(matrix_size))
    ...
    def __call__(self, input: GeometricTensor):
        ...
        input = input.tensor.reshape(input.shape[0], self._C, self.in_type.representations[0].size,  *spatial_shape)

        # compute tensor product
        tensor_features = jnp.einsum('bci...,bco...->bcio...', input, input)
        tensor_features = tensor_features.reshape(input.shape[0], self._C, self.hidden_type.representations[0].size, *spatial_shape)
        ...
        _matrix = _matrix.reshape(self._C, self.out_type.representations[0].size, self.hidden_type.representations[0].size)
        output = jnp.einsum('coi,bci...->bco...', _matrix, tensor_features)

        output = output.reshape(input.shape[0], self.out_type.size, *spatial_shape)
    ...
    def expand_parameters(self) -> Array:
        r"""
        ...
        return _matrix.reshape(self.out_type.size, self.hidden_type.representations[0].size)
    ...
    def train(self, mode=True):
        ...
        if not mode:
            ...
            if hasattr(self, "matrix"):
                new = eqx.tree_at(lambda m: m.matrix, new, replace=_matrix)
        else:
            if hasattr(self, "matrix"):
                # del self.matrix
                new = eqx.tree_at(lambda m: m.matrix, new, replace=None)

        return super(TensorProductModule, new).train(mode)
