...
def _factor_out_endomorphisms(hombasis: np.ndarray, irrep: escnn_jax.group.IrreducibleRepresentation) -> np.ndarray:
...
def _compute_irrep_embeddings(
        representation: Dict[GroupElement, np.matrix],
        irrep: escnn_jax.group.IrreducibleRepresentation,
) -> np.ndarray:
    r"""
...
def decompose_representation_general(
        representation: Callable[[GroupElement], np.matrix],
        group: escnn_jax.group.Group,
        irreps: List[escnn_jax.group.IrreducibleRepresentation] = None
) -> Tuple[np.matrix, List[Tuple[Tuple, int]]]:
    r"""
