...
from escnn_jax.group import change_basis, directsum
from escnn_jax.group.irrep import generate_irrep_matrices_from_generators
...
class Octahedral(Group):
    ...
    def _subgroup(self, id) -> Tuple[
        ...
        if id == ('octa',):
            ...
            sg = escnn_jax.group.cyclic_group(1)
            ...
            sg = escnn_jax.group.cyclic_group(3)
            axis = np.asarray([1., 1., 1.]) / np.sqrt(3)
            parent_map = cn_to_octa(adj, sg, axis=axis)
            child_map = octa_to_cn(adj, sg, axis=axis)
        elif id == (False, 4):
            sg = escnn_jax.group.cyclic_group(4)
            axis = np.asarray([0., 0., 1.])
            axis /= np.linalg.norm(axis)
            parent_map = cn_to_octa(adj, sg, axis=axis)
            child_map = octa_to_cn(adj, sg, axis=axis)
        elif id == (True, 2):
            sg = escnn_jax.group.dihedral_group(2)
            parent_map, child_map = None, None
            raise NotImplementedError()
        elif id == (True, 3):
            sg = escnn_jax.group.dihedral_group(3)
            parent_map, child_map = None, None
            raise NotImplementedError()
        elif id == (True, 4):
            sg = escnn_jax.group.dihedral_group(4)
            parent_map, child_map = None, None
            raise NotImplementedError()
        else:
...
def octa_to_cn(adj: GroupElement, cn: escnn_jax.group.CyclicGroup, axis: np.ndarray):
...
def cn_to_octa(adj: GroupElement, cn: escnn_jax.group.CyclicGroup, axis: np.ndarray):
    assert isinstance(adj.group, Octahedral)
