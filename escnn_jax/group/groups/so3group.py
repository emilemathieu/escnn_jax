...
from escnn_jax.group import directsum, change_basis
        ...
        if sg_id1 == ('so3',):
            ...
            if sg_id2[0] in ['ico', 'tetra']:
                ...
                if n == 3:
                    ...
                    from escnn_jax.group.groups.ico import _PHI
...
def _clebsh_gordan_tensor_so3(m: int, n: int, j: int):
    if py3nj is None:
        return escnn_jax.group._clebsh_gordan._clebsh_gordan_tensor((m,), (n,), (j,), SO3.__name__)
    else:
...
def so3_to_rotgroup(adj: GroupElement, rotgroup: escnn_jax.group.Group):
...
def rotgroup_to_so3(adj: GroupElement, rotgroup: escnn_jax.group.Group):
    assert isinstance(adj.group, SO3)
    assert 'Q' in rotgroup.PARAMETRIZATIONS
...
def so3_to_so2(adj: GroupElement, so2: Union[escnn_jax.group.SO2, escnn_jax.group.CyclicGroup]):
    assert isinstance(adj.group, SO3)
...
def so2_to_so3(adj: GroupElement, so2: Union[escnn_jax.group.SO2, escnn_jax.group.CyclicGroup]):
    assert isinstance(adj.group, SO3)
...
def so3_to_o2(adj: GroupElement, o2: Union[escnn_jax.group.O2, escnn_jax.group.DihedralGroup]):
    assert isinstance(adj.group, SO3)
...
def o2_to_so3(adj: GroupElement, o2: Union[escnn_jax.group.O2, escnn_jax.group.DihedralGroup]):
    assert isinstance(adj.group, SO3)
