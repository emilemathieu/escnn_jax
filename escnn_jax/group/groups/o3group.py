...
from escnn_jax.group import Group, GroupElement
...
from escnn_jax.group import change_basis
from escnn_jax.group.irrep import restrict_irrep
...
class O3(Group):
    ...
    def _subgroup(self, id: Tuple) -> Tuple[
        ...
        if sg_id == (False, 'so3'):
            # SO3 group
            sg = escnn_jax.group.so3_group(self._maximum_frequency)
            ...
            if len(so3_sg_id) == 1 and isinstance(so3_sg_id[0], str):
                ...
                if so3_sg_id == (False, -1):
                    # SO(2) x C_2
                    sg = escnn_jax.group.cylinder_group(self._maximum_frequency)
                    parent_mapping = cyl_to_o3(adj, sg)
                    child_mapping = o3_to_cyl(adj, sg)
                elif so3_sg_id == (True, -1):
                    # Cylinder,  O(2) x C_2
                    sg = escnn_jax.group.full_cylinder_group(self._maximum_frequency)
                    ...
                    if N == 1:
                        ...
                        sg = escnn_jax.group.cyclic_group(2)
                        parent_mapping = inv_to_o3(self, sg)
                        child_mapping = o3_to_inv(self, sg)
                    else:
                        # C_N x C_2
                        sg = escnn_jax.group.cylinder_discrete_group(N)
                        parent_mapping = cyl_to_o3(adj, sg)
                        child_mapping = o3_to_cyl(adj, sg)
                elif so3_sg_id[0] is True and so3_sg_id[1] > 1:
                    # Cylinder, D_N x C_2
                    N = so3_sg_id[1]
                    sg = escnn_jax.group.full_cylinder_discrete_group(N)
                    parent_mapping = fullcyl_to_o3(adj, sg)
                    child_mapping = o3_to_fullcyl(adj, sg)
                elif so3_sg_id[0] is True and so3_sg_id[1] == 1:
            ...
            if len(so3_sg_id) == 1 and isinstance(so3_sg_id[0], str):
                if so3_sg_id[0] == 'ico':
                    sg = escnn_jax.group.ico_group()
                    parent_mapping = so3_to_o3(adj, sg)
                    child_mapping = o3_to_so3(adj, sg)
                elif so3_sg_id[0] == 'octa':
                    sg = escnn_jax.group.octa_group()
                    parent_mapping = so3_to_o3(adj, sg)
                    child_mapping = o3_to_so3(adj, sg)
                elif so3_sg_id[0] == 'tetra':
                ...
                if so3_sg_id == (False, -1):
                    # SO(2)
                    sg = escnn_jax.group.so2_group(self._maximum_frequency)
                    parent_mapping = so2_to_o3(adj, sg)
                    child_mapping = o3_to_so2(adj, sg)
                elif so3_sg_id == (True, -1):
                    # Dihedral,  O(2)
                    sg = escnn_jax.group.o2_group(self._maximum_frequency)
                    ...
                    sg = escnn_jax.group.cyclic_group(n)
                    parent_mapping = so2_to_o3(adj, sg)
                    child_mapping = o3_to_so2(adj, sg)
                    ...
                    n = so3_sg_id[1]
                    assert isinstance(n, int)
                    sg = escnn_jax.group.dihedral_group(n)
    ...
    def bl_quotient_representation(self,
                                   L: int,
                                   subgroup_id,
                                   ) -> escnn_jax.group.Representation:
        r"""
...
def o3_to_fullplato(adj: GroupElement, plato: escnn_jax.group.DirectProductGroup):
...
def fullplato_to_o3(adj: GroupElement, plato: escnn_jax.group.DirectProductGroup):
    assert isinstance(adj.group, O3)
...
def o3_to_so3(adj: GroupElement, so3: Union[escnn_jax.group.SO3, escnn_jax.group.Icosahedral]):
...
def so3_to_o3(adj: GroupElement, so3: Union[escnn_jax.group.SO3, escnn_jax.group.Icosahedral]):
    # can also support Tetra and Octa
...
def o3_to_so2(adj: GroupElement, so2: Union[escnn_jax.group.SO2, escnn_jax.group.CyclicGroup]):
    assert isinstance(adj.group, O3)
...
def so2_to_o3(adj: GroupElement, so2: Union[escnn_jax.group.SO2, escnn_jax.group.CyclicGroup]):
    assert isinstance(adj.group, O3)
...
def o3_to_dih(adj: GroupElement, dih: Union[escnn_jax.group.O2, escnn_jax.group.DihedralGroup]):
    assert isinstance(adj.group, O3)
...
def dih_to_o3(adj: GroupElement, dih: Union[escnn_jax.group.O2, escnn_jax.group.DihedralGroup]):
    assert isinstance(adj.group, O3)
...
def o3_to_con(adj: GroupElement, con: Union[escnn_jax.group.O2, escnn_jax.group.DihedralGroup]):
    assert isinstance(adj.group, O3)
...
def con_to_o3(adj: GroupElement, con: Union[escnn_jax.group.O2, escnn_jax.group.DihedralGroup]):
    assert isinstance(adj.group, O3)
...
def o3_to_mir(adj: GroupElement, mir: escnn_jax.group.CyclicGroup):
    
    assert isinstance(adj.group, O3)
    assert isinstance(mir, escnn_jax.group.CyclicGroup) and mir.order() == 2
...
def mir_to_o3(adj: GroupElement, mir: escnn_jax.group.CyclicGroup):
...
def o3_to_inv(o3: O3, inv: escnn_jax.group.CyclicGroup):
    
    assert isinstance(inv, escnn_jax.group.CyclicGroup) and inv.order() == 2
...
def inv_to_o3(o3: O3, inv: escnn_jax.group.CyclicGroup):
...
def o3_to_cyl(adj: GroupElement, cyl: escnn_jax.group.DirectProductGroup):
    assert isinstance(adj.group, O3)
    assert isinstance(cyl.G1, escnn_jax.group.CyclicGroup) and cyl.G1.order() == 2
    assert isinstance(cyl.G2, escnn_jax.group.CyclicGroup) or isinstance(cyl.G2, escnn_jax.group.SO2)
...
def cyl_to_o3(adj: GroupElement, cyl: escnn_jax.group.DirectProductGroup):
    assert isinstance(adj.group, O3)
    assert isinstance(cyl.G1, escnn_jax.group.CyclicGroup) and cyl.G1.order() == 2
    assert isinstance(cyl.G2, escnn_jax.group.CyclicGroup) or isinstance(cyl.G2, escnn_jax.group.SO2)
...
def o3_to_fullcyl(adj: GroupElement, fullcyl: escnn_jax.group.DirectProductGroup):
    assert isinstance(adj.group, O3)
    assert isinstance(fullcyl.G1, escnn_jax.group.CyclicGroup) and fullcyl.G1.order() == 2
    assert isinstance(fullcyl.G2, escnn_jax.group.DihedralGroup) or isinstance(fullcyl.G2, escnn_jax.group.O2)
...
def fullcyl_to_o3(adj: GroupElement, fullcyl: escnn_jax.group.DirectProductGroup):
    assert isinstance(adj.group, O3)
    assert isinstance(fullcyl.G1, escnn_jax.group.CyclicGroup) and fullcyl.G1.order() == 2
    assert isinstance(fullcyl.G2, escnn_jax.group.DihedralGroup) or isinstance(fullcyl.G2, escnn_jax.group.O2)
