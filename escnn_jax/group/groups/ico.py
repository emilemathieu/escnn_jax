...
class Icosahedral(Group):
    ...
    def _subgroup(self, id) -> Tuple[
        ...
        if id == ('ico',):
            ...
            sg = escnn_jax.group.cyclic_group(5)
            ...
            sg = escnn_jax.group.dihedral_group(5)
            parent_map, child_map = None, None
            raise NotImplementedError()
        else:
...
def ico_to_cn(adj: GroupElement, cn: escnn_jax.group.CyclicGroup, axis: np.ndarray):
...
def cn_to_ico(adj: GroupElement, cn: escnn_jax.group.CyclicGroup, axis: np.ndarray):
    assert isinstance(adj.group, Icosahedral)
