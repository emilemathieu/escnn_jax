...
class TestPooling(TestCase):
    ...
    def test_orientation_pooling(self):
        ...
        x = jax.random.normal(key, (3, r.size, 10, 15))

        x = GeometricTensor(x, r)

        for el in g.testing_elements:
            
            out1 = np.array(mpl(x).transform_fibers(el).tensor)
            out2 = np.array(mpl(x.transform_fibers(el)).tensor)
    
            errs = (out1 - out2)
            errs = np.abs(errs).reshape(-1)
            print(el, errs.max(), errs.mean(), errs.var())
    
            assert jnp.allclose(out1, out2, atol=1e-6, rtol=1e-5), \
                'The error found during equivariance check with element "{}" is too high: max = {}, mean = {} var ={}' \
                    .format(el, errs.max(), errs.mean(), errs.var())
